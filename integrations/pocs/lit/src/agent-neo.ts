...
@customElement('agent-neo')
export class AgentNeoElement extends LitElement {
  @property({ attribute: false }) config: AppConfig = { endpoints: [] } as AppConfig;
  @property({ type: String }) preset = 'default';
  @property({ type: String }) context = '';
  @property({ attribute: false }) user: User | null = null;

  @state() private isOpen = false;
  @state() private agentState: 'idle' | 'active' | 'thinking' = 'idle';
  @state() private messages: Message[] = [];
  @state() private input = '';
  @state() private isTyping = false;
  @state() private currentStep?: InteractionStep;
  @state() private workflowState: Record<string, any> = {};
  @state() private lastActionResult: any = null;
  @state() private showOptions = true;
  @state() private theme: 'dark' | 'light' | 'glass' = 'dark';
  @state() private showThemeMenu = false;
  @state() private showGreeting = false;
  ...
  updated(changed: Map<string, unknown>) {
    if (changed.has('config') || changed.has('preset')) {
    ...
    if (changed.has('isTyping')) {
      this.agentState = this.isOpen ? (this.isTyping ? 'thinking' : 'active') : 'idle';
    }

    if (changed.has('currentStep')) {
      this.autoExecuteStepIfNeeded();
    }

    if (changed.has('messages') || changed.has('isTyping') || changed.has('showOptions')) {
      this.scrollToBottom();
    }
  }
  ...
  private renderAnimatedNexus() {
    ...
    return html`
      ...
      const pulse = this.agentState === 'thinking'
        ...
        : 'none';
            const stroke = getStrokeColor(hex.id);
            return html`
              <path
                class="nexus-hex"
  ...
  private renderAvatar() {
    return html`
      ...
      <div class="floating-avatar glass-morphism" @click=${() => (this.isOpen = !this.isOpen)}>
    ...
    return html`
      <div class="chat-container glass-morphism-dark" data-theme=${this.theme}>
        <div class="chat-header">
          ...
          <div class="header-controls">
            <div class="theme-selector-container">
              <button class="close-btn" title="Change Theme" @click=${() => (this.showThemeMenu = !this.showThemeMenu)}>
        ...
        ? html`
            ...
            (t) => html`
                          <button class="theme-option" @click=${() => this.toggleTheme(t.id as any)}>
        ...
        ? html`
                <div class="options-container">
                  ${this.currentStep.options.map(
          (opt) => html`
                      <button class="option-pill" @click=${() => this.handleOptionClick(opt)}>${opt.label}</button>
                    `
        )}
                </div>
              `
        : nothing}
          ...
          <div class="actions-suggestion">
            ...
            <button class="action-button" @click=${() => this.handleQuickAction()}>
        ...
        if (e.key === 'Enter') this.handleSend();
          ...
          />
          <button class="send-btn" @click=${() => this.handleSend()} ?disabled=${this.isTyping}>
...
export type { AgentNeoElement as AgentNeoLitElement };
